...
class DeepSeekStreamHandler:
    ...
    async def handle_stream_async(self, response) -> AsyncGenerator[str, None]:
        ...
        is_thinking_model = (
            "think" in self.model.lower()
            or "r1" in self.model.lower()
            or bool(self.reasoning_effort)
        )
        is_silent_model = "silent" in self.model.lower()
        is_fold_model = (
            "fold" in self.model.lower()
            or "search" in self.model.lower()
    ...
    def _process_chunk(
        ...
        if (
            chunk.get("v")
            and isinstance(chunk["v"], dict)
            and chunk["v"].get("response")
        ):
        ...
        # Handle accumulated token usage
            ...
            for item in chunk["v"]:
                if item.get("p") == "accumulated_token_usage" and isinstance(
                    item.get("v"), (int, float)
                ):
        ...
        if isinstance(chunk.get("v"), str):
            ...
            content = "".join(
                item.get("content", "")
                for item in chunk.get("v", "")
                if isinstance(item, dict)
            )
        else:
            content = str(chunk.get("v", "")) if chunk.get("v") is not None else ""
    ...
    def _send_content(
        ...
        cleaned = re.sub(
            r"^(SEARCH|WEB_SEARCH|SEARCHING)\s*", "", cleaned, flags=re.IGNORECASE
        )
        ...
        if path in ["content", ""]:
            ...
            tool_calls: list[dict[Any, Any]] = ToolParser.parse_tool_calls_from_text(
                self.tool_call_buffer
            )
            if tool_calls:
                self.has_tool_call = True
                delta: dict[str, list[dict[Any, Any]] | str] = {
        ...
        if path == "thinking":
            ...
            if is_fold_model:
                if not self.thinking_started:
                    ...
                    delta["content"] = (
                        f"<details><summary>Thinking Process</summary><pre>{cleaned}"
                    )
                else:
    ...
    def _handle_done(
        self, is_fold_model: bool, is_search_silent_model: bool
    ) -> Optional[str]:
        """Handle stream end"""
        delta = {}
        ...
        if self.search_results and not is_search_silent_model:
            ...
            for result in sorted(
                self.search_results, key=lambda x: x.get("cite_index", 0)
            ):
    ...
    async def handle_non_stream_async(self, response) -> Dict:
        ...
        async for line in response.aiter_lines():
            ...
            try:
                ...
                if (
                    parsed.get("v")
                    and isinstance(parsed["v"], dict)
                    and parsed["v"].get("response")
                ):
                    ...
                    for fragment in fragments:
                        ...
                        content = re.sub(
                            r"^(SEARCH|WEB_SEARCH|SEARCHING)\s*",
                            "",
                            content,
                            flags=re.IGNORECASE,
                        )
                ...
                # Handle accumulated token usage
                elif parsed.get("p") == "response" and isinstance(
                    parsed.get("v"), list
                ):
                    for item in parsed["v"]:
                        if item.get("accumulated_token_usage") and isinstance(
                            item.get("v"), (int, float)
                        ):
                            self.accumulated_token_usage = int(item["v"])
        ...
        clean_content, tool_calls = ToolParser.parse_tool_calls_from_text_with_content(
            accumulated_content
        )
