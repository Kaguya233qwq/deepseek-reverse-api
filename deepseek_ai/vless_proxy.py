...
class VlessProxy:
    ...
    async def create_connection(
        ...
        try:
            ...
            if self.config.tls:
                ...
                reader.set_transport(ssl_transport)  # pyright: ignore[reportArgumentType]
                writer = asyncio.StreamWriter(ssl_transport, protocol, reader, loop)  # pyright: ignore[reportArgumentType]
            ...
            raise ConnectionError(
                f"Connection to Vless server {self.config.address}:{self.config.port} timed out"
            )
        except Exception as e:
    ...
    async def test_connection(
        ...
        try:
            reader, writer = await asyncio.wait_for(
                self.create_connection(target_host, target_port), timeout=timeout
            )
...
class VlessProxyPool:
    ...
    async def test_all_proxies(
        self, target_host: str = "www.google.com", target_port: int = 443
    ) -> Dict[str, bool]:
        """
        测试所有代理
